...
type PopoverPlacement = { top: number; left?: number; right?: number };
function placementFor(button: HTMLElement): PopoverPlacement {
  const rect = button.getBoundingClientRect();
  const top = rect.bottom + 4;
  const roomRight = window.innerWidth - rect.left;
  if (roomRight >= popoverWidthPx + 16) return { top, left: rect.left };
  return { top, right: Math.max(8, window.innerWidth - rect.right) };
}

interface ModelCaveatBadgeProps {
  symbol: string;
  /** This ticker's own caveats; the badge is not rendered when there are none (2026-09-24). */
  caveats: RoadmapItem[];
}

/**
 * The "model" badge (Signals page badges column and the modal header, 2026-09-24): shown only when
 * the ticker has a caveat of its own, its popover lists just those — the measures every ticker is
 * waiting on live in the page's roadmap panel instead, so they are not repeated on every row.
 */
export function ModelCaveatBadge({ symbol, caveats }: ModelCaveatBadgeProps) {
  ...
  return (
    <div ref={containerRef} style={{ display: "inline-block" }}>
      <button
        ...
        aria-label={`${symbol}: model caveats`}
      ...
      <div className={`dropdown-menu p-3 ${isOpen ? "show" : ""}`} style={{ position: "fixed", ...placement, width: `${popoverWidthPx}px`, maxWidth: "calc(100vw - 2rem)", overflowY: "auto", whiteSpace: "normal", zIndex: 1100 }}>
        {caveats.map((caveat, index) => (
          <div key={caveat.id} className={index === caveats.length - 1 ? "" : "mb-2"}>
